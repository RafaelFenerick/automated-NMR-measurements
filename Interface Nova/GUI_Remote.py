...
from PyQt5.QtWidgets import QApplication, QWidget, QToolTip, QPushButton, QMessageBox, QDesktopWidget, \
    ...
    QSizePolicy, QRadioButton, QCheckBox, QFileDialog
...
class GUI(QMainWindow):
    ...
    def initUI(self):
        ...
        addapplicationAct.triggered.connect(self.close)  # Ação para fechar interface
    ...
    def new_experiment(self):
        ...
        self.amostrapath.setText("C:\\Users\\Rafael Fenerick\\Documents\\Git\\pnmr\\Interface Nova")
        ...
        for application in self.applications_text:
            check = QCheckBox(application.strip("\n"))
    ...
    def temperaturechoose(self, value):
        ...
        if value == 0:
            ...
            self.experimenttemperature1.setText("340")
        ...
        if value == 1:
            ...
            self.steptemperature1.setText("5")
            ...
            self.endtemperature1.setText("340")
...
class Experiment:
    ...
    def start(self, gasflow, to_tune, low_temperature):
        ...
        postrequest(str([3, gasflow, low_temperature, to_tune]))
        #    self.to_break = True
    ...
    def run(self, temperature, wait_time, applications, do_ramp, low_temperature, calib, init=False):
        ...
        if temperature != 0:
            ...
            if (abs(current_temperature - temperature) >= do_ramp[1] and do_ramp[0]) or init:
                ...
                postrequest(str([4, temperature, 15, wait_time]))
                #    self.to_break = True
                #    self.control.AbortApplication()
                #    self.control.Finish(low_temperature=low_temperature)
                #    write_log("Término de execução em: " + str(temperature) + " K com falha em GetTemperature")
                ...
                postrequest(str([5, temperature, wait_time]))
                #    self.to_break = True
                #    self.control.AbortApplication()
                #    self.control.Finish(low_temperature=low_temperature)
    ...
    def end(self, low_temperature, calib):
        ...
        postrequest(str([7, low_temperature]))
        #    self.to_break = True
...
def postrequest(args):
    ...
    strargs = {"data" : str(args)}
    url = " http://127.0.0.1:7500/receiver"
    request = Request(url, urlencode(strargs).encode())
